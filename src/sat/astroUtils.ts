...
export function toJulianDate(date: Date): number {
  ...
  const day =
    utcDay +
    (utcHour + (utcMinute + (utcSecond + utcMs / 1000.0) / 60.0) / 60.0) / 24.0;
...
export function greenwichSiderealTime(jd: number): number {
  ...
  let gmst =
    24110.54841 +
    tCent * (8640184.812866 + tCent * (0.093104 - tCent * 6.2e-6));
...
export function sunEciKm(jd: number): [number, number, number] {
  ...
  let l = (280.46 + 0.9856474 * n) % 360.0;
  if (l < 0) {
    l += 360.0;
  }
  ...
  const gr = g * DEG2RAD;
  const lambda =
    lr + (1.915 * Math.sin(gr) + 0.02 * Math.sin(2 * gr)) * DEG2RAD;
...
export function moonEciKm(jd: number): [number, number, number] {
  ...
  const argumentOfLatitudeDeg = normalizeDeg(93.272095 + 483202.0175233 * t);
